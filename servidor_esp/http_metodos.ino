...
void GET_HORA(){
  ...
  //http.begin("http://192.168.1.113:3001/api/v1/esp32/hora"); //HTTP
  http.begin(String(server) + "hora"); //HTTP
...
void POST_JSON_REGISTRO(uint8_t num ,String payload, String clase_api, String tipo_api ){
  ...
  http.begin(String(server) + clase_api + "/" + tipo_api);
  Serial.println("API: " + String(server) + clase_api + "/" + tipo_api);
...
//  http.begin("http://192.168.1.113:3001/api/v1/esp32/status"); //HTTP
  http.begin(String(server) + "status"); //HTTP
